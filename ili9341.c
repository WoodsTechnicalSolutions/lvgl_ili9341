 ...
 * @file ili9341.c
 *
 * This is the driver that I contributed to LVGL.
 *
 ...
 */
void ili9341_write(int mode, uint8_t byte)
{
	io_dc(mode);
	io_spi_write(byte);
}
...
void ili9341_init(int argc, char *argv[], int32_t lcd_h, int32_t lcd_v)
{
	...
	/* hardware reset */
	//io_cs(1); // would need to be created, if needed
	io_dc(1);
	io_reset(0);
	io_delay_us(50);
	io_reset(1);
...
void ili9341_flush(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
	...
	if (!disp) {
		printf("error disp == NULL\n");
		return;
	}

	lcd_h = lv_display_get_horizontal_resolution(disp);
	lcd_v = lv_display_get_vertical_resolution(disp);

	if (area->x2 < 0 || area->y2 < 0 || area->x1 > (lcd_h - 1) || area->y1 > (lcd_v - 1)) {
	...
	act_x2 = area->x2 > lcd_h - 1 ? lcd_h - 1 : area->x2;
	act_y2 = area->y2 > lcd_v - 1 ? lcd_v - 1 : area->y2;

	len = (act_x2 - act_x1 + 1) * 2;
	w = (area->x2 - area->x1) + 1;
	...
	for (y = act_y1; y <= act_y2; y++) {
		ili9341_write_array(ILI9341_DATA_MODE, px_map, len);
		px_map += w;
	}
